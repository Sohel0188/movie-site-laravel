...
class CountryBrowseService
{
    ...
    public function query(string $countryCode, int $page, string $mediaType = 'all'): array
    {
        ...
        if ($mediaType === 'movie') {
            $result = $this->pagination->paginateDiscover(
                $page,
                $perPage,
                fn ($p) => $this->tmdb->fetch("/discover/movie?with_origin_country={$code}&sort_by=popularity.desc&page={$p}"),
                fn ($r) => $this->mapItem($r, 'movie'),
            );

            return array_merge($result, ['country_code' => $code]);
        }
        ...
        if ($mediaType === 'tv') {
            $result = $this->pagination->paginateDiscover(
                $page,
                $perPage,
                fn ($p) => $this->tmdb->fetch("/discover/tv?with_origin_country={$code}&sort_by=popularity.desc&page={$p}"),
        ...
        $movieMeta = $this->tmdb->fetch("/discover/movie?with_origin_country={$code}&sort_by=popularity.desc&page=1");
        $tvMeta = $this->tmdb->fetch("/discover/tv?with_origin_country={$code}&sort_by=popularity.desc&page=1");
        ...
        $result = $this->pagination->paginateMerged(
            ...
            function ($p) use ($code) {
                try {
                    $movies = $this->tmdb->fetch("/discover/movie?with_origin_country={$code}&sort_by=popularity.desc&page={$p}");
                } catch (\Throwable) {
                    $movies = ['results' => []];
                }
                try {
                    $tv = $this->tmdb->fetch("/discover/tv?with_origin_country={$code}&sort_by=popularity.desc&page={$p}");
                } catch (\Throwable) {
                    $tv = ['results' => []];
                }
                $items = array_merge(
